...
class Int4XPUIntegerZPLayout(Layout):
    """Only for PyTorch version at least 2.7"""

    pass


@register_layout(Int4XPUIntegerZPLayout)
class Int4XPUIntegerZPAQTTensorImpl(AQTTensorImpl):
    """
    TensorImpl for int4 XPU Integer Zero-Point layout for affine quantized tensor, this is for int4 only,
    ...
    def __tensor_unflatten__(
        ...
        return cls(packed_weight, transposed, _layout, scale, zero)
    ...
    def from_plain(
        ...
        assert isinstance(_layout, Int4XPUIntegerZPLayout)
    ...
    def to(self, *args, **kwargs):
        ...
        if not is_device(torch.device(self.device).type, device):
            raise ValueError(
                f"Int4XPUIntegerZPAQTTensorImpl does not support conversion from {self.device} to {device}"
            )
    ...
    def __torch_dispatch__(cls, func, types, args, kwargs):
        ...
        if func is aten.t.default:
            ...
            """
            transposed = Int4XPUIntegerZPAQTTensorImpl(
