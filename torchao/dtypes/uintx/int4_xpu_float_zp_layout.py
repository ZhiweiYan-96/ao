...
def _linear_bf16_act_uint4_weight_float_zero_check(input_tensor, weight_tensor, bias):
    return (
        ...
        and isinstance(weight_tensor._layout, Int4XPUFloatZPLayout)
    )
...
class Int4XPUFloatZPLayout(Layout):
    """Only for PyTorch version at least 2.7"""

    pass


@register_layout(Int4XPUFloatZPLayout)
class Int4XPUFloatZPAQTTensorImpl(AQTTensorImpl):
    """
    ...
    def from_plain(
        ...
        assert isinstance(_layout, Int4XPUFloatZPLayout)
        ...
        if TORCH_VERSION_AT_LEAST_2_8:
            ...
            ), "torch.ops.aten._convert_weight_to_int4pack_for_xpu expects `int32` dtype"
    ...
    def to(self, *args, **kwargs):
        ...
        if not is_device(torch.device(self.device).type, device):
            raise ValueError(
                f"Int4XPUFloatZPLayout does not support conversion from {self.device} to {device}"
            )
    ...
    def __torch_dispatch__(cls, func, types, args, kwargs):
        ...
        if func is aten.t.default:
            ...
            """
            transposed = Int4XPUFloatZPAQTTensorImpl(
